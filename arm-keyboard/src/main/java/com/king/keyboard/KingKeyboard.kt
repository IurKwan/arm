...
open class KingKeyboard : LifecycleEventObserver {
    ...
    constructor(context: Context, rootView: ViewGroup, keyboardParentView: ViewGroup?) :
        this(
            ...
            keyboardContainer =
                LayoutInflater
                    .from(context)
                    .inflate(R.layout.king_keyboard_container, null),
     ...
     * @param xmlLayoutResId 键盘布局的资源文件，其中包含键盘布局和键值码等相关信息
     */
    fun setKeyboardCustomModeChange(
        @XmlRes xmlLayoutResId: Int,
    ) {
     ...
     * 键盘输入法是否显示
     */
    fun isShow(): Boolean = keyboardContainer.isVisible
     ...
     */
    fun getKeyboardView(): KingKeyboardView = keyboardView

    /**
     * 对外提供获取KingKeyboardView的配置
     */
    fun getKeyboardViewConfig(): KingKeyboardView.Config = keyboardView.getConfig()
     ...
     */
    private fun isSoundEffectEnabled(): Boolean = isPlaySoundEffect
    ...
    private fun setSoundEffectEnabled(soundEffectEnabled: Boolean) {
        ...
        if (ContextCompat.checkSelfPermission(
            ...
            Settings.System.putInt(
                ...
                if (soundEffectEnabled) 1 else 0,
            )
        } else {
            Log.w(
                TAG,
                "${context.packageName} was not granted this permission: android.permission.WRITE_SETTINGS.",
            )
        }
    }
     ...
     * 是否开启震动
     */
    fun isVibrationEffectEnabled(): Boolean = isVibrationEffect
