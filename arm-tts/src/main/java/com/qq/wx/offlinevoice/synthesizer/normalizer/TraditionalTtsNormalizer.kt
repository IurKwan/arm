...
object TraditionalTtsNormalizer {
    ...
    // 2) 精準詞彙替換表（鍵值長度需相等）
    private val RAW_WORD_REPLACEMENTS =
        mapOf(
            // 「著」 Zhuó/Zháo
            "執著" to "執卓",
            "膠著" to "膠卓",
     ...
     * Token 內部深度處理函數
     */
    private fun processInsideToken(
        token: String,
        out: StringBuilder,
        isDebug: Boolean,
    ) {
    ...
    // 簡單判斷是否純標點（可根據需求擴展）
    private fun isPunctuationOnly(s: String): Boolean = s.all { !Character.isLetterOrDigit(it) }
}
