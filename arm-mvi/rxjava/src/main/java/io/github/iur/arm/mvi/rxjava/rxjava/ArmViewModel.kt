...
import io.github.iur.arm.mvi.DeliveryMode
import io.github.iur.arm.mvi.MavericksViewModel
import io.github.iur.arm.mvi.RedeliverOnStart
import io.github.iur.arm.mvi._internal
import io.github.iur.arm.mvi._internal1
import io.github.iur.arm.mvi._internal2
import io.github.iur.arm.mvi._internal3
import io.github.iur.arm.mvi._internal4
import io.github.iur.arm.mvi._internal5
import io.github.iur.arm.mvi._internal6
import io.github.iur.arm.mvi._internal7
import io.github.iur.arm.mvi._internalSF
import io.github.iur.arm.mvi.common.Async
import io.github.iur.arm.mvi.common.Fail
import io.github.iur.arm.mvi.common.InternalMavericksApi
import io.github.iur.arm.mvi.common.Loading
import io.github.iur.arm.mvi.common.MavericksBlockExecutions
import io.github.iur.arm.mvi.common.MavericksState
import io.github.iur.arm.mvi.common.Success
...
abstract class ArmViewModel<S : MavericksState>(
    ...
    ) = toObservable().execute(autoDisposable, { it }, null, stateReducer)
     ...
     */
    fun <T : Any, V> Single<T>.execute(
        autoDisposable: Boolean = true,
        mapper: (T) -> V,
        stateReducer: S.(Async<V>) -> S,
    ) = toObservable().execute(autoDisposable, mapper, null, stateReducer)
    ...
    fun <T : Any> Observable<T>.execute(
        autoDisposable: Boolean = true,
        stateReducer: S.(Async<T>) -> S,
    ) = execute(autoDisposable, { it }, null, stateReducer)

    /**
     * Helper to map a [Completable] to an [Async] property on the state object.
     */
    fun Completable.execute(stateReducer: S.(Async<Unit>) -> S) = toSingle { Unit }.execute(true, stateReducer)
    ...
    private fun logStateChanges() {
        subscribe {
            printState("New State: $it")
        }
    }

    protected open fun printState(content: String) {
        // log util
    }
    ...
    fun subscribe(
        ...
        subscriber: (S) -> Unit,
    ): Disposable = _internal(owner, deliveryMode) { subscriber(it) }.toDisposable()
    ...
    ) = _internal4(null, prop1, prop2, prop3, prop4, RedeliverOnStart) { a, b, c, d ->
        subscriber(
            a,
            b,
            c,
