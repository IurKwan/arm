...
abstract class MavericksRepository<S : MavericksState>(
        ...
        if (config.performCorrectnessValidations) {
            ...
            stateStore.set {
                ...
                if (firstState != secondState) {
                    ...
                    val changedProp =
                        firstState::class.java.declaredFields
                            .asSequence()
     ...
     */
    protected fun onEach(action: suspend (S) -> Unit) = _internal(action)
    ...
    fun <T : Any> Flow<T>.resolveSubscription(action: suspend (T) -> Unit): Job =
        (coroutineScope + config.subscriptionCoroutineContextOverride).launch(start = CoroutineStart.UNDISPATCHED) {
