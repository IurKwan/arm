...
fun assertMavericksDataClassImmutability(
    ...
    val disallowedFieldCollectionTypes =
        listOfNotNull(
 ...
 */
internal class MutableStateChecker<S : MavericksState>(
    val initialState: S,
) {
    data class StateWrapper<S : MavericksState>(
        val state: S,
    ) {
        private val originalHashCode = hashCode()

        fun validate() =
            require(originalHashCode == hashCode()) {
