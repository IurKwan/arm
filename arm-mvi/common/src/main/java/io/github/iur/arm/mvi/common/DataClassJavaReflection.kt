...
fun <T : Any> callCopy(
    ...
    val copyFunction =
        jvmClass.declaredMethods.first { method ->
            method.isSynthetic &&
                method.name.let { name ->
                    name == "copy\$default" || (name.startsWith("copy-") && name.endsWith("\$default"))
                }
 ...
 */
private fun <T : Any> buildPropertyIndexMap(
    jvmClass: Class<*>,
    instance: T,
): Map<String, Int> {
    val componentRegex = Regex("""component(\d+).*""")
    val componentMethods =
        jvmClass.declaredMethods
            .mapNotNull { method ->
