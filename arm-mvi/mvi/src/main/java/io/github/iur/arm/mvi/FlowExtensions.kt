...
import io.github.iur.arm.mvi.common.MavericksTestOverrides
...
internal fun <T : Any?> Flow<T>.collectLatest(
    ...
    val flow =
        when {
            ...
            deliveryMode is UniqueOnly -> {
                this
                    .assertOneActiveSubscription(
                        lifecycleOwner,
                        activeSubscriptions,
                        deliveryMode.subscriptionId,
                    ).dropWhile {
                        lastDeliveredStates.containsKey(deliveryMode.subscriptionId) &&
                            it == lastDeliveredStates[deliveryMode.subscriptionId]
    ...
    val scope =
        lifecycleOwner.lifecycleScope + Mavericks.viewModelConfigFactory.subscriptionCoroutineContextOverride
