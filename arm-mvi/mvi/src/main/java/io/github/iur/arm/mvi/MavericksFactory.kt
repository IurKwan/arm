...
private fun <VM : MavericksViewModel<S>, S : MavericksState> createViewModel(
    ...
    val factoryViewModel =
        viewModelClass.factoryCompanion()?.let { factoryClass ->
            try {
                factoryClass
                    .getMethod(
                        "create",
                        ViewModelContext::class.java,
                        MavericksState::class.java,
    ...
    val viewModel =
        requireNotNull(factoryViewModel ?: createDefaultViewModel(viewModelClass, initialState)) {
            if (viewModelClass.constructors
                    .firstOrNull()
                    ?.parameterTypes
                    ?.size
                    ?.let { it > 1 } == true
...
private fun <VM : MavericksViewModel<S>, S : MavericksState> createDefaultViewModel(
    ...
    if (viewModelClass.constructors.size == 1) {
        ...
        if (primaryConstructor.parameterTypes.size == 1 &&
            primaryConstructor.parameterTypes[0].isAssignableFrom(
                state::class.java,
            )
        ) {
            if (!primaryConstructor.isAccessible) {
                try {
                    ...
                    throw IllegalStateException(
                        "ViewModel class is not public and Mavericks could not make the primary constructor accessible.",
                        e,
                    )
                }
            }
...
open class ViewModelDoesNotExistException(
    message: String,
