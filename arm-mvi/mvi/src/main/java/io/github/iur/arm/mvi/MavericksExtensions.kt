 ...
 *
 * Looks for [io.github.iur.arm.mvi.Mavericks.KEY_ARG] on the arguments of the fragments.
 */
...
inline fun <T, reified VM : MavericksViewModel<S>, reified S : MavericksState> T.viewModel(
    viewModelClass: KClass<VM> = VM::class,
    crossinline keyFactory: () -> String = { viewModelClass.java.name },
): Lazy<VM> where T : ComponentActivity =
    lifecycleAwareLazy(this) {
        MavericksViewModelProvider.get(
            ...
            viewModelContext =
                ActivityViewModelContext(
                    this,
                    intent.extras?.get(Mavericks.KEY_ARG),
                ),
            key = keyFactory(),
        )
    }
            ...
            if (value == null) {
                ...
                argUntyped
                    ?: throw IllegalArgumentException(
                        "Mavericks arguments not found at key _root_ide_package_.com.airbnb.mvrx.Mavericks.KEY_ARG!",
                    )
