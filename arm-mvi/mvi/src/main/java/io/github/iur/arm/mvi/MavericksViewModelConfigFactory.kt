...
import io.github.iur.arm.mvi.common.CoroutinesStateStore
...
open class MavericksViewModelConfigFactory(
    ...
    open fun coroutineScope(): CoroutineScope = CoroutineScope(SupervisorJob() + Dispatchers.Main.immediate + contextOverride)

    internal fun <S : MavericksState> provideConfig(
        viewModel: MavericksViewModel<S>,
        initialState: S,
    ): MavericksViewModelConfig<S> =
        buildConfig(viewModel, initialState).also { config ->
