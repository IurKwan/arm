...
class MavericksViewInternalViewModel(
    state: SavedStateHandle,
) : ViewModel() {
    internal val lastDeliveredStates: ConcurrentHashMap<String, Any?> =
        ConcurrentHashMap<String, Any?>()
    internal val activeSubscriptions: MutableSet<String> = mutableSetOf()
    internal val mavericksViewId =
        state[PERSISTED_VIEW_ID_KEY] ?: generateUniqueId().also { id ->
